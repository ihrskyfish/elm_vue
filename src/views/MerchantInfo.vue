...
const merchantId = route.params.merchantId; // 正确获取路由参数
const merchant = ref<Merchant | null>(null);
const foods = ref<Food[]>([]);
const cartItems = ref<CartItem[]>([]);

const MERCHANT_API_URL = `/api/merchant/merchantes/${merchantId}`;
const FOODS_API_URL = `/api/food/lists/${merchantId}`;
const CART_API_URL = `/api/cart/lists`;

const fetchData = async () => {
  try {
    const merchantResponse = await axios.get(MERCHANT_API_URL);
    if (merchantResponse && merchantResponse.data && merchantResponse.data.data) {
      merchant.value = merchantResponse.data.data;
    }
...
const fetchCart = async () => {
  try {
    ...
    if (cartResponse && cartResponse.data && cartResponse.data.data) {
      ...
      // 将购物车数据与商家数据进行合并
      cartArr.forEach(cartItem => {
        cartItem.merchantVo = merchant.value;
      });
...
const createOrder = async (merchantId: number, daId: number, orderTotal: number) => {
  try {
...
const handleCreateOrder = async (merchantId: number, daId: number, orderTotal: number) => {
  try {
    // console.log(merchantId,daId,orderTotal);
    const result = await createOrder(merchantId, daId, orderTotal);
...
<template>
  ...
  <body class="">
    <div class="w-full h-full ">
      ...
      <!--商家图片部分-->
      <div v-if="merchant" class="w-full h-full mt-28 flex justify-center items-center text-base">
        <img :src="merchant.merchantImg || '/src/assets/img/default.png'" class="w-72 h-72 rounded">
      </div>

      <!--商家信息部分-->
      <div v-if="merchant" class="w-full h-32 flex flex-col justify-center items-center text-2xl">
        <h1 class="text-4xl font-semibold ">{{ merchant.merchantName }}</h1>
        <p class="text-2xl font-medium text-gray-500 mt-px">&#165;{{ merchant.starPrice }}起送
          &#165;{{ merchant.deliveryPrice }}配送</p>
        <p class="text-2xl font-medium text-gray-500 mt-px">{{ merchant.merchantExplain }}</p>
      </div>
      ...
      <div class="w-full h-28 fixed left-0 bottom-0 flex text-base">
        ...
        <div class="basis-1/3">
          <button
            ...
            @click="totalCartPrice > 0 && handleCreateOrder(Number(merchantId), 1, totalCartPrice)">
            去结算
          </button>
        </div>
      </div>
