...
onMounted(async () => {
  try {
    const response = await axios.get('/api/merchant/merchantLists');
    if (response && response.data && response.data.data) {
      merchantes.value = response.data.data as Merchant[];
    }
  } catch (error) {
    ...
    <div class="wrapper">
      ...
      <div class="merchant-list">
        <RouterLink v-for="merchant in merchantes" :key="merchant.merchantId"
          ...
          <div class="merchant">
            <img :alt="merchant.merchantName" :src="merchant.merchantImg" />
            <div class="merchant-info">
              <div class="merchant-name">
                <h3>{{ merchant.merchantName }}</h3>
                <i class="fa fa-mobile-phone" style="font-size: 4vw"></i>
              </div>
              <div class="merchant-info-start">
                <!-- 星级和月售等信息 -->
                <div class="merchant-info-left">
                  ...
                  <i class="fa fa-star"></i>
                  <p>
                    4.9&nbsp;月售{{ merchant.orderQuantity }}单
                  </p>
                </div>
                <div class="merchant-info-right" style="line-height: 1">
                  蜂鸟专送
                </div>
              </div>
              <div class="merchant-delivery">
                <p>{{ merchant.starPrice }}元起送</p>
                <p>1km</p>
              </div>
              <div class="merchant-info-detail">
                <div>{{ merchant.merchantExplain }}</div>
              </div>
              <div class="merchant-info-promotion">
                <div class="merchant-info-promotion-left">
                  <div class="merchant-info-promotion-left-icon">新</div>
                  <p>饿了么新用户首单立减9元</p>
                </div>
                <div class="merchant-info-promotion-right">
                  <p>2个活动&nbsp;&nbsp;</p>
                  <i class="fa fa-caret-down"></i>
                </div>
              </div>
              <div class="merchant-info-promotion">
                <div class="merchant-info-promotion-left">
                  <div class="merchant-info-promotion-left-icon" style="background-color: orangered">特</div>
                  <p>特价商品5元起</p>
                </div>
              </div>
            </div>
