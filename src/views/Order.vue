  ...
  <body>
    <div class="w-full h-full">
        ...
        <el-table :data="tableData" style="width: 100%">
          ...
          <el-table-column align="right" fixed="right">
            <template #default="scope">
              <el-button size="small" @click="handleEdit(scope.$index, scope.row)">选择
              </el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <div v-for="order in orders" :key="order.orderId" class="w-full">
        <p
          class="box-border p-[3vw] text-3xl font-bold text-[#666666FF] border-solid border-b-[1px] border-b-[#DDDDDDFF]">
          {{ order.merchantVo ? order.merchantVo.merchantName : '' }}</p>
        ...
        <ul v-for="detail in order.list" :key="detail.odId" class="w-full">
          <li class="w-full h-32 box-border p-[3vw] text-[#666666FF] flex justify-between items-center ">
            <div class="flex items-center">
              <img :src="detail.food ? detail.food.foodImg : ''" height="40" width="40" />"
        ...
        <RouterLink :to="`/payment/${orderId}`" class="block w-full h-full">
          <div class="w-full h-28 fixed left-0 bottom-0 flex">
            <div
              ...
              &#165;{{ order.orderTotal + (order.merchantVo?.deliveryPrice || 0) }}
            </div>
            <div
